...
public class Part3 {
    ...
    public static void main(String[] args)
    {
        ...
        int n = 0;
        Part5Model model = new Part5Model();
	...
	for(int i=0;i<n;i++)
		{table[i]=scan.nextLine();
                  model.addRoutedAddress(table[i]);}
	...
	int f = Integer.parseInt(r_split[i][3]);
	int num[]={0,0,0,0};
	int c=7,g=0;
	...
	//the result of the routing operation is then printed
        String route = "";
	if(j==isplit.length)
	{   route = r_split[i][r_split[i].length-2]+" "+r_split[i][r_split[i].length-1];
            // System.out.println("Route to : "+r_split[i][r_split[i].length-2]+" "+r_split[i][r_split[i].length-1]);
            model.setChosenInterface(route);
		break;}	}

        String route2 = "";
	if(i==n-1)
	{   route2 = r_split[i][r_split[i].length-2]+" "+r_split[i][r_split[i].length-1];
            // System.out.println("Route to : "+r_split[i][r_split[i].length-2]+" "+r_split[i][r_split[i].length-1]);
            model.setChosenInterface(route2);
        }
            
            Part5 part5 = new Part5(model);
